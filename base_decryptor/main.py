...
@click.option('-f', '--file', type=click.File('r', encoding='utf-8'), help="Specify a file containing encoded strings")  # File input
@click.option('-a', '--all', is_flag=True, help="Show decoding results for all encodings")  # Show all possible decodings
@click.option('-q', '--quiet', is_flag=True, help="Suppress the ASCII banner and author message")  # Suppress banner and message
def input_handler(encoded_string, file, all, quiet):
    """
    The Base Decryptor: Automatically detect and decode base-encoded strings.
